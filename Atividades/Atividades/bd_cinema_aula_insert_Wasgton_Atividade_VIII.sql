...
create database bd_cinema_aula_insert_Wasgton_Atividade_VIII;
use bd_cinema_aula_insert_Wasgton_Atividade_VIII;
...
#1.       Crie a tabela SEXO com os atributos ID e nome.
...
#2.       Insira os sexos existentes na tabela SEXO.
...
#3.       Relacione a tabela SEXO com a tabela CLIENTE.

alter table cliente add id_sex_fk int;
alter table cliente add foreign key(id_sex_fk) references sexo(id_sex) ;


#4.       Relacione a tabela SEXO com a tabela FUNCIONARIO.

alter table funcionario add id_sex_fk int;
alter table funcionario add foreign key(id_sex_fk) references sexo(id_sex) ;

#5.       Utilizando os valores existes no atributo sexo da tabela CLIENTE, crie códigos para atualizar a FK de sexo na tabela CLIENTE.

update cliente set id_sex_fk = 1 where (sexo_cli="Feminino");
update cliente set id_sex_fk = 2 where (sexo_cli="Masculino");


#6.       Utilizando os valores existes no atributo sexo da tabela FUNCIONARIO, crie códigos para atualizar a FK de sexo na tabela FUNCIONARIO.

update funcionario set id_sex_fk = 1 where (sexo_fun="Feminino");
update funcionario set id_sex_fk = 2 where (sexo_fun="Masculino");

#7.       Sabendo que agora o atributo sexo na tabela CLIENTE não é mais necessário, pois essa informação está sendo fornecida pelo atributo FK de sexo, exclua o atributo sexo da tabela CLIENTE.

alter table cliente drop sexo_cli ;


#8.       Sabendo que agora o atributo sexo na tabela FUNCIONARIO não é mais necessário, pois essa informação está sendo fornecida pelo atributo FK de sexo, exclua o atributo sexo da tabela FUNCIONARIO.

alter table funcionario drop sexo_fun ;

#9.       Crie a tabela TIPO_SANGUINEO com os atributos ID, fator e rh.

create table Tipo_Sanguineo(
id_tip int primary key auto_increment,
fator_tip varchar(100),
rh_tip varchar(50)
);

#10.   Insira na tabela TIPO_SANGUINEO os valores existentes correspondente à fator e rh sanguíneo.
...
#11.   Relacione a tabela TIPO_SANGUINEO com a tabela CLIENTE.

alter table cliente add id_tipo_sanguineo_fk int;
alter table cliente add foreign key(id_tipo_sanguineo_fk) references Tipo_sanguineo(id_tip) ;

#12.   Utilizando os valores existes no atributo tipagem sanguíneo da tabela CLIENTE, crie códigos para atualizar a FK de tipo sanguíneo na tabela CLIENTE.

update cliente set id_tipo_sanguineo_fk = 1 where (tipagem_sanguinea_cli="A+");
update cliente set id_tipo_sanguineo_fk = 2 where (tipagem_sanguinea_cli="B+");
update cliente set id_tipo_sanguineo_fk = 3 where (tipagem_sanguinea_cli="AB+");
update cliente set id_tipo_sanguineo_fk = 4 where (tipagem_sanguinea_cli="O+");
update cliente set id_tipo_sanguineo_fk = 5 where (tipagem_sanguinea_cli="A-");
update cliente set id_tipo_sanguineo_fk = 6 where (tipagem_sanguinea_cli="B-");
update cliente set id_tipo_sanguineo_fk = 7 where (tipagem_sanguinea_cli="AB-");
update cliente set id_tipo_sanguineo_fk = 8 where (tipagem_sanguinea_cli="O-");

#13.   Sabendo que agora o atributo tipagem sanguínea na tabela CLIENTE não é mais necessário, pois essa informação está sendo fornecida pelo atributo FK de tipo sanguíneo, exclua o atributo tipagem sanguíneo da tabela CLIENTE.

alter table cliente drop tipagem_sanguinea_cli ;

#14.   Crie a tabela FUNÇÃO com os atributos ID, nome.
...
#15.   Insira na tabela FUNCAO os valores existentes correspondente a função. Analise os dados existente na tabela FUNCIONÁRIO para saber quais são.

#select funcao_fun from funcionario group by funcao_fun;
...
#16.   Relacione a tabela FUNÇÃO com a tabela FUNCIONÁRIO.

alter table funcionario add id_funcao_fk int;
alter table funcionario add foreign key(id_funcao_fk) references funcao(id_func) ;

#17.   Utilizando os valores existes no atributo função da tabela FUNCIONÁRIO, crie códigos para atualizar a FK de função na tabela FUNCIONÁRIO.

update funcionario set id_funcao_fk = 1 where (funcao_fun="Atendente");
update funcionario set id_funcao_fk = 2 where (funcao_fun="Gerente");
update funcionario set id_funcao_fk = 3 where (funcao_fun="Segurança");
update funcionario set id_funcao_fk = 4 where (funcao_fun="Supervisor");
update funcionario set id_funcao_fk = 5 where (funcao_fun="Zelador");
update funcionario set id_funcao_fk = 6 where (funcao_fun="Auxiliar");
update funcionario set id_funcao_fk = 7 where (funcao_fun="Diretor");
update funcionario set id_funcao_fk = 8 where (funcao_fun="Contador");


#18.   Sabendo que agora o atributo função na tabela FUNCIONÁRIO não é mais necessário, pois essa informação está sendo fornecida pelo atributo FK de função, exclua o atributo função da tabela FUNCIONÁRIO.

alter table funcionario drop funcao_fun ;

#19.   Crie a tabela CLASSIFICAÇÃO com os atributos ID, nome.

create table Classificacao(
id_cla int primary key auto_increment,
 nome_cla varchar(200)
);

#20.   Insira na tabela CLASSIFICAÇÃO os valores existentes correspondente a classificação. Analise os dados existente na tabela FILME para saber quais são.

#select classificacao_indicativa_film from filme group by classificacao_indicativa_film;
...
#21.   Relacione a tabela CLASSIFICAÇÃO com a tabela FILME.

alter table filme add id_classificacao_fk int;
alter table filme add foreign key(id_classificacao_fk) references classificacao(id_cla) ;

#22.   Utilizando os valores existes no atributo classificação indicativa da tabela FILME, crie códigos para atualizar a FK de classificação na tabela FILME.

update filme set id_classificacao_fk = 1 where (classificacao_indicativa_film="Livre");
update filme set id_classificacao_fk = 2 where (classificacao_indicativa_film="10+");
update filme set id_classificacao_fk = 3 where (classificacao_indicativa_film="14+");
update filme set id_classificacao_fk = 4 where (classificacao_indicativa_film="16+");
update filme set id_classificacao_fk = 5 where (classificacao_indicativa_film="18+");

#23.   Sabendo que agora o atributo classificação indicativa na tabela FILME não é mais necessário, pois essa informação está sendo fornecida pelo atributo FK de classificação, exclua o atributo classificação da tabela FILME.

alter table filme drop classificacao_indicativa_film ;

#24.   Mostre os dados dos clientes do sexo masculino, nascidos entre os anos 1960 e 1990 e com e-mail do provedor Hotmail ou Outlook ou Gmail, ordenados por nome.

select * from cliente where (id_sex_fk=2)and( data_nasc between ('1960-01-01')and('1990-12-31'))and((email_cli like ("%Hotmail%"))or (email_cli like ("%Outlook%"))or (email_cli like ("%Gmail%")) ) order by nome_cl;

#25.   Mostre quantos clientes do sexo feminino existem no sistema, personalize o resultado com AS.

select count(cliente.id_sex_fk ) as "Quantidade de clientes do sexo feminino" from cliente,sexo where (cliente.id_sex_fk = 1) and (sexo.nome_sex='Feminino');

#26.   Mostre quantos clientes do sexo masculino existem no sistema, personalize o resultado com AS.

select count(cliente.id_sex_fk ) as "Quantidade de clientes do sexo feminino" from cliente,sexo where (cliente.id_sex_fk = 2) and (sexo.nome_sex='Masculino');

#27.   Mostre quantos clientes moram em Ruas.

select count(endereco_cli) as "Quantidade de clientes cadastrados que moram em uma rua" from cliente where (endereco_cli like  "Rua%");

#28.   Mostre o valor médio das despesas cadastradas no sistema no primeiro semestre de 2021.

select avg(valor_desp) as  " Valor médio das despesas cadastradas no sistema no primeiro semestre de 2021" from despesa where (data_desp between ("2021-01-01")and("2021-06-30"));

#29.   Mostre a despesa com o valor mais alto cadastrada no sistema no segundo semestre de 2021.

select MAX(valor_desp) as  "Despesa com o valor mais alto cadastrada no sistema no segundo semestre de 2021" from despesa where (data_desp between ("2021-06-30")and("2021-12-31"));

#30.   Mostre os dados do funcionário com o maior salário.

select * from funcionario where salario_fun=(select max(salario_fun) as "Funcionário com o maior salário" from funcionario);

#31.   Mostre o valor médio do salário dos funcionários com a função atendente.

select avg(funcionario.salario_fun) as " O valor médio do salário dos funcionários com a função atendente  " from funcionario, funcao where (funcionario.id_funcao_fk=1)and(funcao.nome_func= "Atendente");

#32.   Mostre a soma do salário dos funcionários com a função zelador e do sexo masculino.

select sum(funcionario.salario_fun) as " A soma do salário dos funcionários com a função zelador e do sexo masculino  " from funcionario, funcao, sexo where (funcionario.id_funcao_fk=5)and(funcao.nome_func= "Zelador");

select *from funcao;

#33.   Mostre os dados dos funcionários que trabalham no período noturno (18h as 23h59), ordenados pelo nome de forma decrescente.

select* from funcionario where (horario_entrada_fun = "18:00:00") and (horario_saida_fun >= "23:00:00")  ;

#34.   Mostre quais funções diferentes de funcionários existem no sistema, ordene pelo nome da função.

  select ( funcao.nome_func ) from funcionario, funcao where (funcionario.id_funcao_fk= funcao.id_func) group by funcao.nome_func order by funcao.nome_func ;

#35.   Mostre quantas funções diferentes de funcionários existem no sistema.

  select count( funcao.id_func ) as "Funções diferentes de funcionários existem no sistema" from  funcao;

#36.   Mostre os dados dos filmes que possui o gênero terror, ordene pelo título.

  select * from filme where (genero_film like "%Terror%") order by titulo_film;

#37.   Mostre os dados dos filmes com a classificação maior de 18 anos e ordene pelo título de forma decrescente.

  select * from filme, classificacao where (filme.id_classificacao_fk= classificacao.id_cla)and (classificacao.nome_cla="18+") order by filme.titulo_film ;

#38.   Mostre os dados dos filmes que classificação entre 14 e 18 anos e com o gênero Comédia ou Ação ou Drama.

  select * from filme, classificacao where (filme.id_classificacao_fk= classificacao.id_cla)and (classificacao.nome_cla between ("14+")and("18+"))and((filme.genero_film like "%com_dia%")or(filme.genero_film like "%ação%")or(filme.genero_film like "%drama%")) order by filme.titulo_film ;

#39.   Mostre os dados do filme com a maior duração.

select * from filme where duracao_film= (select  max(duracao_film)from filme);

#40.   Mostre o valor máximo, valor mínimo, valor médio, a soma dos valores e a quantidade de produtos existentes no sistema.

select*from produto;

select max(valor_venda_prod) as " Maior valor do produto cadastrado " from produto;
select min(valor_venda_prod) as " Menor valor do produto cadastrado " from produto;
select avg(valor_venda_prod) as " Média de valores dos produtos cadastrados" from produto;
select sum(valor_venda_prod) as " Soma dos valores dos produtos cadastrados " from produto;
select count(id_prod) as " Quantidade de produtos existentes no sistema " from produto;
