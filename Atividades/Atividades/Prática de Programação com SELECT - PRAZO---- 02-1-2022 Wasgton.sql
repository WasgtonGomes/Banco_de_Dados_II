...
#Prática de Programação com SELECT - PRAZO: 02/11/2022 
...
CREATE TABLE Pagamentos (
    ...
    FOREIGN KEY (id_desp_fk)
        REFERENCES Despesas (id_desp),
    FOREIGN KEY (id_comp_fk)
        REFERENCES Compra_produto (id_comp)
);


#Atividade de Fixação
#Utilize o Banco de Dados da Mecânica e faça os exercícios a seguir:
...
#3. Selecione os clientes com os sobrenomes Silva ou Souza, do sexo
#feminino, com renda entre 500 e 10.000 reais e nascidos após 01/01/1980 ;

select * from cliente where (((nome_cli like "%Silva%")or(nome_cli like "%Souza%"))
and(id_sex_fk = 2 )and(renda_familiar_cli between(500)and(10000))and(data_nascimento_cli > "1980-01-01"));
...
select  id_cli, nome_cli, estado_civil_cli, cpf_cli, rg_cli, data_nascimento_cli, max(renda_familiar_cli),telefone_cli, celular_cli,id_sex_fk,id_end_fk from cliente ;
...
SELECT * FROM Cliente GROUP BY estado_civil_cli;
...
SELECT count(cor_car),cor_car FROM carro GROUP BY cor_car;
...
select * from cliente where (renda_familiar_cli< '35543.2818359375') ;
...
select max(quant_prod) from produto;
select avg(quant_prod) from produto;
select sum(quant_prod) from produto;

#15. Mostre o menor valor, maior valor, valor médio e soma do valor dos produtos;

select min(valor_prod) from produto;
select max(valor_prod) from produto;
select avg(valor_prod) from produto;
select sum(valor_prod) from produto;

#16. Mostre o nome e o estoque do produto com o menor estoque;

select descricao_prod ,min(quant_prod) from produto;

#17. Mostre o nome e o estoque do produto com o maior estoque;

select descricao_prod ,max(quant_prod) from produto;

#18. Mostre o nome, a função e o salario do funcionário com menor salario;

select nome_func , funcao_func, min(salario_func) from funcionario where (salario_func = 900 );

#19. Mostre o nome, a função e o salario do funcionário com maior salario;

select nome_func , funcao_func, max(salario_func) from funcionario where (salario_func = 45000 );
...
select * from servico where (((tipo_serv = "conserto")or(tipo_serv="estética"))and (valor_serv > 10.00)and (valor_serv < '853.1179996490479'));
