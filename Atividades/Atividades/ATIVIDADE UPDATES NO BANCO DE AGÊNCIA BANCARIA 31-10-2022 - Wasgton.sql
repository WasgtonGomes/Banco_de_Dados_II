...
#   ATIVIDADE UPDATES NO BANCO DE AGÊNCIA BANCARIA


create database  BD_Banco_Agencia_Bancaria_Updates_Wasgton;
use BD_Banco_Agencia_Bancaria_Updates_Wasgton;

create table Banco (
cod_ban int  not null primary key ,
...
insert into  Cliente values (5, 'Lívia de Souza', '333.444.666-98', '0033333 sesdec/RO', 'F', '1982-01-30', 1100.00, 'Av. Ji-Paraná', 'livia.souza@hotmail.com', '8498 9898');
...
cod_cc int not null primary key ,
...
insert into  Conta_Corrente values  (2, 41897, '2009-01-30', 2200.00, 5000.00, 4.00, 1, 2);
...
create table Saque (
cod_saq int not null primary key,
...
cod_trans int not null primary key,
...
/*
Atividade de Fixação – BD Agência Bancária*/

#1. Crie um update que mude o nome do cliente nº 5 para ‘Lucas Matos’;

update cliente set nome_cli="Lucas Matos" where (cod_cli = 5);

#2. Crie um update que aumentar a renda em 20% dos clientes nascidos
#antes do ano 2000;

UPDATE cliente SET renda_cli = (renda_cli * 1.20) WHERE (data_nascimento_cli <="2000-01-01");

#3. Crie um update para diminuir a renda em 27% dos clientes homens com
#renda maior ou igual a 5000 reais;
...
UPDATE cliente SET data_nascimento_cli =("1982-10-10") WHERE (cod_cli = 2);

#5. Altere para 50 a idade dos clientes com ID entre 2 e 5 ou nascidos entre
#1970 e 1980;

UPDATE cliente SET data_nascimento_cli =("1972-10-02") WHERE (((cod_cli >= 2) and(cod_cli<=5))
 or ((data_nascimento_cli between ("1970-01-01")and ("1980-12-31"))));

#6. Diminua 500 reais na renda dos clientes nascidos entre 1980 e 2000 que
#tenha renda superior a 1000 reais e idade menos que 50;

UPDATE cliente SET renda_cli =(renda_cli - 500) WHERE ( ((data_nascimento_cli between ("1980-01-01")and ("2000-12-31")) and (renda_cli > 1000) and (data_nascimento_cli>"1972-12-31")));

#7. Aumente 1000 reais na renda dos clientes com idade entre 10 e 20
#anos ou 40 e 60 anos.

UPDATE cliente SET renda_cli =(renda_cli + 1000) WHERE ( ((data_nascimento_cli between ("2002-01-01")and ("2012-12-31")) or ((data_nascimento_cli between ("1962-01-01")and ("1982-12-31")))));

#8. Aumente a taxa mensal em 30% das conta corrente com saldo
#maior do que 500 reais e data de abertura no primeiro semestre de
#2009;

UPDATE Conta_Corrente SET taxa_mensal_cc =(taxa_mensal_cc * 1.3) WHERE ( (saldo_cc > 500) and (data_abertura_cc between ("2009-01-01")and("2009-12-31")));

#9. Zere a taxa mensal das conta corrente com saldo superior ou igual a
#1000 reais.

UPDATE Conta_Corrente SET taxa_mensal_cc =(0) WHERE ( (saldo_cc >= 1000));

#10. Aumente o valor do limite em 50% das contas com data de aberta
#anterior a 2014 ou com valor de limite inferior a 1000 reais.

select * from Conta_Corrente;
UPDATE Conta_Corrente SET valor_limite_cc =(valor_limite_cc * 1.5) WHERE ( (data_abertura_cc < "2014-01-01")and(valor_limite_cc < 1000));
select * from Conta_Corrente;
