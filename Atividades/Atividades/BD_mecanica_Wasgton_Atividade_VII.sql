/* 
Scrit do BD_Mecanica Carro ZERO -- Atividade VII 
 */
 #Turma: 3º Período ADS 2022/02
#Discente: Wasgton Gomes Pereira

create database BD_mecanica_Wasgton_Atividade_VII ;
use BD_mecanica_Wasgton_Atividade_VII ;
...
UPDATE venda SET valor_vend = (valor_vend * 0.85) WHERE ( (forma_pag_vend ='A vista'));
select *from venda;
...
select count(cod_cli) as "Quantidade de clientes cadastrados" from cliente;

#Mostre a maior renda entre os clientes cadastrados no banco de dados.

select max(renda_cli) as "Maior renda dos clientes cadastrados" from cliente;

#Mostre o salário médio entre os funcionários cadastrados no banco de dados.

select avg(salario_func)  as "Média salarial dos funcionario cadastrados"from funcionario;

#Mostre a soma do estoque de todos os produtos cadastrado no banco de dados.

select sum(estoque_prod)  as "Soma de todos os produtos cadastrados"from produto;

#Mostre o menor estoque entre os produtos cadastrados no banco de dados.

select min(estoque_prod)  as "Menor estoque dos produtos cadastrados"from produto;
...
select sum(valor_vend) from venda where (data_vend between ('2011-01-01')and('2018-12-31'))  ;
...
delete from recebimento where (valor_rec>500 )and(data_vencimento_rec between ('2017-01-01')and('2017-06-30'));
